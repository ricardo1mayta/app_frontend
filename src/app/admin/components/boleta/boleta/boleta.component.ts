...
export class BoletaComponent implements OnInit {
  ...
  serieComprobante: tDoc[] = [
    { value: 'B001', viewValue: 'B001' },
    { value: 'B002', viewValue: 'B002' },
    { value: 'B003', viewValue: 'B003' },
    { value: 'B004', viewValue: 'B004' },
  ];
  tipoDoc: tDoc[] = [
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      if (idpedido > 0) {
        ...
        this.pedidosService.getPedidoId(idpedido).subscribe((ped) => {
          this.pedidosService.getDetallePedidoId(idpedido).subscribe((det) => {
            ...
            this.calculototales();
            this.form.controls['cod_tipo_documento'].setValue('03');
          });
        });
      }
    });
    ...
    this.form.controls['cliente_tipodocumento'].setValue('1');
  ...
  calculototales() {
    ...
    if (parseInt(this.descuentos) >= 0) {
      if (this.dataSource.length >= 0) {
        ...
        this.total = parseFloat((tt - parseFloat(this.descuentos)).toFixed(2));

        this.suttotal = (tt / 1.18).toFixed(2);
        this.igv = (this.total - this.total / 1.18).toFixed(2);
        this.total_letras =
          'SON ' + this.numeroLetras.NumeroALetras(this.total, 'SOLES');
